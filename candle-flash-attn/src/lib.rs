...
/// batches with a paged K/V cache — the batched decode entry point.
///
/// This fuses the exact eager op (`QK^T*scale + additive_mask` + per-head sink
/// logit + max-subtract + softmax + drop-sink + `@V`) for a batch of sequences
/// of differing lengths whose K/V lives in a physical paged cache. Each query
/// row is processed against its own sequence's KV (sliding window + selected
/// compressed blocks), and fully-masked 64-column blocks are skipped.
///
/// # Arguments
///
/// * `q` - Query tensor `(total_q, num_heads_q, head_size)`, BF16, contiguous.
/// * `k` - Paged key tensor `(num_blocks, page_block_size, num_heads_kv, head_size)`,
///   BF16, contiguous.
/// * `v` - Paged value tensor with the same shape as `k`, BF16, contiguous.
/// * `seqlens_q` - Cumulative query lengths `[B + 1]`, I32, on CUDA.
/// * `seqlens_k` - Cumulative KV lengths `[B + 1]`, I32, on CUDA.
/// * `block_table` - Paged block table `(B, max_blocks)`, I32, on CUDA (physical
///   block index per `(sequence, block-in-sequence)`).
/// * `block_bias_mask` - Additive attention mask `(total_q, max_kv)`, F32, `0`/`-inf`.
/// * `sink_logits` - Per-head sink logits `(num_heads_q)`, F32.
/// * `page_block_size` - Tokens per K/V cache block (multiple of 64).
/// * `softmax_scale` - Scaling factor for `QK^T` (e.g. `head_size^-0.5`).
///
/// MQA/GQA is supported (`num_heads_q % num_heads_kv == 0`). `head_size` must be a
/// power of two and at most 512. Result shape is `(total_q, num_heads_q, head_size)`.
pub fn flash_attn_varlen_paged_windowed_blockmask(
    q: &Tensor,
    k: &Tensor,
    v: &Tensor,
    seqlens_q: &Tensor,
    ...
    let op = FlashAttnVarLenPagedBlockMask {
        softmax_scale,
        seqlens_q: seqlens_q.clone(),
        seqlens_k: seqlens_k.clone(),
...
/// Custom-op3 wrapper for the variable-length / paged DeepSeek V4 DSA flash
/// kernel [`flash_attn_varlen_paged_windowed_blockmask`]. Holds the cumulative
/// seqlens, the per-sequence paged block table, the materialized per-query
/// additive mask and the per-head sink logits; `cuda_fwd` launches the fused
/// BF16 kernel. This is the batched continuous-batching decode entry point.
pub struct FlashAttnVarLenPagedBlockMask {
    pub softmax_scale: f32,
    /// Cumulative query lengths `[B + 1]`, I32, on CUDA.
    pub seqlens_q: Tensor,
    /// Cumulative KV lengths `[B + 1]`, I32, on CUDA.
    pub seqlens_k: Tensor,
    /// Paged block table `[B, max_blocks]`, I32, on CUDA.
    pub block_table: Tensor,
    /// Additive attention mask `[total_q, max_kv]`, F32, `0`/`-inf`.
    pub block_bias_mask: Tensor,
    /// Per-head sink logits `[H]`, F32.
    pub sink_logits: Tensor,
...
impl FlashAttnVarLenPagedBlockMask {
    fn cuda_fwd(
        &self,
        q: &candle::CudaStorage,
        q_l: &Layout,
        ...
        if !q_l.is_contiguous() || !k_l.is_contiguous() || !v_l.is_contiguous() {
            candle::bail!(
                "flash-attn-varlen-paged-blockmask expects contiguous q/k/v \
                 (got layouts q={q_l:?}, k={k_l:?}, v={v_l:?})"
            )
        }
        ...
        let (num_blocks, page_block_size, num_heads_k, k_head_size) = k_l.shape().dims4()?;
        if v_l.shape().dims4()? != k_l.shape().dims4()? {
            candle::bail!("shape mismatch k {:?} and v {:?}", k_l.shape(), v_l.shape())
        }
        ...
        if head_size > 512 {
            candle::bail!(
                "flash-attn-varlen-paged-blockmask only supports head size at most 512 (got {head_size})"
            )
        }
        if !head_size.is_power_of_two() {
            candle::bail!(
                "flash-attn-varlen-paged-blockmask requires a power-of-two head size (got {head_size})"
            )
        }
        if num_heads % num_heads_k != 0 {
            candle::bail!(
        ...
        if page_block_size % 64 != 0 {
            candle::bail!(
                "flash-attn-varlen-paged-blockmask requires page_block_size a multiple of 64 (got {page_block_size})"
            )
        }

        let (seqlens_q, sq_l) = self.seqlens_q.storage_and_layout();
        let seqlens_q = match &*seqlens_q {
            candle::Storage::Cuda(c) => c.as_cuda_slice::<i32>()?,
            _ => {
                candle::bail!("flash-attn-varlen-paged-blockmask requires a CUDA seqlens_q tensor")
            }
        };
        let seqlens_q = seqlens_q.slice(sq_l.start_offset()..);
        let (seqlens_k, sk_l) = self.seqlens_k.storage_and_layout();
        let seqlens_k = match &*seqlens_k {
            candle::Storage::Cuda(c) => c.as_cuda_slice::<i32>()?,
            _ => {
                candle::bail!("flash-attn-varlen-paged-blockmask requires a CUDA seqlens_k tensor")
            }
        };
        let seqlens_k = seqlens_k.slice(sk_l.start_offset()..);
        let (block_table, bt_l) = self.block_table.storage_and_layout();
        let block_table = match &*block_table {
            candle::Storage::Cuda(c) => c.as_cuda_slice::<i32>()?,
            _ => candle::bail!(
                "flash-attn-varlen-paged-blockmask requires a CUDA block_table tensor"
            ),
        };
        let block_table = block_table.slice(bt_l.start_offset()..);
        let (mask, mask_l) = self.block_bias_mask.storage_and_layout();
        let mask = match &*mask {
            candle::Storage::Cuda(c) => c.as_cuda_slice::<f32>()?,
            _ => candle::bail!("flash-attn-varlen-paged-blockmask requires a CUDA mask tensor"),
        };
        let mask = mask.slice(mask_l.start_offset()..);
        let (sink, sink_l) = self.sink_logits.storage_and_layout();
        let sink = match &*sink {
            candle::Storage::Cuda(c) => c.as_cuda_slice::<f32>()?,
            _ => candle::bail!("flash-attn-varlen-paged-blockmask requires a CUDA sink tensor"),
        };
        let sink = sink.slice(sink_l.start_offset()..);

        let nseqlens = sq_l.shape().dims1()?;
        if nseqlens < 2 {
            candle::bail!("seqlens_q should have a len >= 2 {nseqlens}")
        }
        if sk_l.shape().dims1()? != nseqlens {
            candle::bail!("seqlens_q and seqlens_k should have the same number of elements")
        }
        let batch_size = nseqlens - 1;
        let (_, max_blocks) = self.block_table.dims2()?;
        // Mask is [total_q, max_kv]; the kernel skips columns past each seq's kvlen.
        let max_kv = self.block_bias_mask.dim(1)?;
        if total_q != q_l.shape().dims()[0] {
            candle::bail!("internal total_q mismatch")
        }
        let _ = num_blocks; // block_table indices validated implicitly by the kernel
        ...
        unsafe {
            ...
            let (k_ptr, _guard) = k.device_ptr(&stream);
            let (v_ptr, _guard) = v.device_ptr(&stream);
            let (seq_q_ptr, _guard) = seqlens_q.device_ptr(&stream);
            let (seq_k_ptr, _guard) = seqlens_k.device_ptr(&stream);
            let (bt_ptr, _guard) = block_table.device_ptr(&stream);
            let (mask_ptr, _guard) = mask.device_ptr(&stream);
            let (sink_ptr, _guard) = sink.device_ptr(&stream);
            let (dst_ptr, _guard) = dst.device_ptr_mut(&stream);
            ffi::flash_attn_varlen_paged_blockmask(
                q_ptr as *const core::ffi::c_void,
                k_ptr as *const core::ffi::c_void,
                v_ptr as *const core::ffi::c_void,
                seq_q_ptr as *const i32,
                seq_k_ptr as *const i32,
                bt_ptr as *const i32,
                mask_ptr as *const core::ffi::c_void,
                sink_ptr as *const core::ffi::c_void,
                dst_ptr as *const core::ffi::c_void,
...
impl candle::CustomOp3 for FlashAttnVarLenPagedBlockMask {
    fn name(&self) -> &'static str {
        "flash-attn-varlen-paged-blockmask"
    }
    ...
    fn cpu_fwd(
        ...
        candle::bail!("no cpu support for flash-attn-varlen-paged-blockmask")
    }
    ...
    fn cuda_fwd(
        ...
        match q.dtype() {
            candle::DType::BF16 => self.cuda_fwd(q, q_l, k, k_l, v, v_l),
            dt => candle::bail!(
                "flash-attn-varlen-paged-blockmask is only supported for bf16 (got {dt:?})"
            ),
        }
    }
}
