...
/// D]` (one query row per sequence), paged `k`/`v` `[num_blocks,
/// page_block_size, Hk, D]` (K == V), `seqlens_q`/`seqlens_k` `[B + 1]`
/// cumulative, `block_table` `[B, max_blocks]`, additive `block_bias_mask`
...
fn flash_attn_blockmask_varlen_paged(
    ...
    candle_flash_attn::flash_attn_varlen_paged_windowed_blockmask(
        q,
        k,
        v,
        seqlens_q,
...
/// Per-layer paged continuous-batching state for [`DeepseekV4Attention`]:
/// per-sequence sliding-window local KV (K == V) and per-sequence DSA
/// compression state. The combined `[sliding | compressed]` KV of every
/// sequence is scattered into physical pages at kernel time.
#[derive(Debug, Clone)]
struct DeepseekV4PagedLayer {
    /// Per-sequence trimmed sliding-window local KV, `[len_i, 1, head_dim]` BF16.
    local_kv: Vec<Option<Tensor>>,
    /// Per-sequence DSA compression state.
    comp_state: Vec<DeepseekV4CompressorState>,
}
...
impl DeepseekV4PagedLayer {
    fn new(cfg: &DeepseekV4Config, layer_idx: usize, num_sequences: usize) -> Self {
        let comp_state = match cfg.layer_types[layer_idx] {
            ...
            LayerType::CompressedSparseAttention => vec![
                DeepseekV4CompressorState::Csa {
    ...
    fn clear(&mut self) {
        for l in self.local_kv.iter_mut() {
            *l = None;
        }
        for c in self.comp_state.iter_mut() {
            *c = match c {
...
/// A physical paged K/V cache for batched continuous-batching decode of a
/// `DeepseekV4ForCausalLM`. One cache is shared by all layers; each layer keeps
/// per-sequence sliding KV and compression state, and the combined
/// `[sliding | compressed]` KV of every sequence is scattered into physical
/// pages of `page_block_size` tokens (a multiple of 64) addressed through a
/// per-sequence block table at kernel time. This is the serving-style batched
/// decode entry point that drives the varlen/paged DSA flash kernel
/// (`flash_attn_varlen_paged_windowed_blockmask`).
#[derive(Debug, Clone)]
pub struct DeepseekV4PagedKvCache {
    page_block_size: usize,
    layers: Vec<DeepseekV4PagedLayer>,
}

impl DeepseekV4PagedKvCache {
    /// Create a cache for `num_sequences` sequences sized to `cfg`, with
    /// physical pages of `page_block_size` tokens (a multiple of 64).
    pub fn new(cfg: &DeepseekV4Config, num_sequences: usize, page_block_size: usize) -> Self {
        assert!(
            page_block_size.is_multiple_of(64),
            "DeepseekV4PagedKvCache requires page_block_size a multiple of 64 (got {page_block_size})"
        );
    ...
    pub fn page_block_size(&self) -> usize {
    ...
    fn layer_mut(&mut self, li: usize) -> &mut DeepseekV4PagedLayer {
        &mut self.layers[li]
    }
}

impl DeepseekV4Attention {
    /// Paged continuous-batching decode (Sq = 1) for a batch of sequences, each
    /// at its own absolute position, whose K/V lives in the physical paged cache
    /// `cache`. Per-sequence position-dependent work (RoPE, compression) runs
    /// per sequence; each sequence's combined `[sliding | compressed]` KV is
    /// scattered into pages and the whole batch is attended in ONE call to
    /// `flash_attn_varlen_paged_windowed_blockmask`. `xs` is `[B, 1, D]` (one
    /// new token per sequence); `positions[B]` are their absolute positions.
    /// Returns the batched decode output `[B, 1, D]`.
    fn forward_paged_decode(
        &mut self,
        xs: &Tensor,
        positions: &[usize],
        cache: &mut DeepseekV4PagedLayer,
        page_block_size: usize,
    ) -> Result<Tensor> {
        let bs = xs.dim(0)?;
        if xs.dim(1)? != 1 {
            candle::bail!(
                "forward_paged_decode requires Sq = 1 for batched decode (got {})",
                xs.dim(1)?
            );
        }
        if positions.len() != bs {
            candle::bail!(
                "forward_paged_decode: {} positions for batch {bs}",
                positions.len()
            );
        }
        let num_heads = self.cfg.num_attention_heads;
        ...
        // Per-sequence: query, KV token, sliding-cache update, compression.
        let mut q_rows: Vec<Tensor> = Vec::with_capacity(bs); // [H, D]
        let mut combined: Vec<Tensor> = Vec::with_capacity(bs); // [kv_len_i, 1, D]
        ...
        for (bi, &pos) in positions.iter().enumerate() {
            ...
            let q = self
                ...
                .transpose(1, 2)?; // [1,H,1,D]
            let q = self.q_b_norm.forward(&q)?;
            let q = self.rotary_emb.forward(&q, variant, pos)?; // [1,H,1,D]
            q_rows.push(q.squeeze(0)?.squeeze(1)?.contiguous()?); // [H, D]

            // Shared KV head (K == V) for this token.
            let kv = self
                .kv_norm
                .forward(&self.kv_proj.forward(&x)?)?
                .reshape((1, 1, 1, head_dim))?
                .transpose(1, 2)?; // [1,1,1,D]
            let kv = self.rotary_emb.forward(&kv, variant, pos)?; // [1,1,1,D]
            let kv_tok = kv.squeeze(0)?.squeeze(0)?; // [1, D]

            // Sliding cache: store the trimmed window, attend over the full one.
            let full_local = match &cache.local_kv[bi] {
                None => kv_tok.unsqueeze(1)?, // [1, 1, D]
                Some(prev) => Tensor::cat(&[prev, &kv_tok.unsqueeze(1)?], 0)?, // [len+1, 1, D]
            };
            cache.local_kv[bi] = Some(self.trim_paged(&full_local)?);
            let local_len = full_local.dim(0)?;

            // Combined KV + per-column mask (all sliding columns are in-window).
            let mut kv_parts: Vec<Tensor> = vec![full_local.clone()];
            let mut mask_part: Vec<f32> = vec![0.0f32; local_len];
            if let Some(comp) = &mut self.compressor {
                let (ckv, bb) =
                    comp.paged_forward(&x, &q_residual, pos, &mut cache.comp_state[bi])?;
                // ckv [1,1,T,D], bb [1,1,1,T]
                kv_parts.push(ckv.squeeze(0)?.transpose(0, 1)?.contiguous()?); // [T, 1, D]
                let bb_flat = bb.squeeze(0)?.squeeze(0)?.squeeze(0)?.to_vec1::<f32>()?; // [T]
                mask_part.extend(bb_flat);
            }
            let combined_i = Tensor::cat(&kv_parts, 0)?.contiguous()?; // [kv_len_i, 1, D]
            let kv_len_i = combined_i.dim(0)?;
            combined.push(combined_i);
            kv_lens.push(kv_len_i);
            mask_rows.push(mask_part);
        }

        // Scatter the combined KV into physical pages (K == V).
        let max_blocks = kv_lens
            .iter()
            .map(|&l| l.div_ceil(page_block_size))
            .max()
            .unwrap_or(0);
        let mut k_pages: Vec<Tensor> = Vec::new();
        let mut block_table: Vec<Vec<i32>> = vec![Vec::new(); bs];
        for bi in 0..bs {
        ...
        let k_paged = Tensor::stack(&k_pages, 0)?.contiguous()?; // [num_blocks, pbs, 1, D]

        // Query stack [B, H, D]; additive mask [B, max_kv] over combined columns.
        let q_stacked = Tensor::stack(&q_rows, 0)?.contiguous()?;
        let max_kv = kv_lens.iter().copied().max().unwrap_or(0);
        let mut mask_vec = vec![f32::NEG_INFINITY; bs * max_kv];
        for bi in 0..bs {
            for (j, m) in mask_rows[bi].iter().enumerate() {
                mask_vec[bi * max_kv + j] = *m;
            }
        }
        let mask = Tensor::from_vec(mask_vec, (bs, max_kv), dev)?;

        // Cumulative seqlens and per-sequence block table.
        ...
        let seqlens_q = Tensor::from_vec(seqlens_q, (bs + 1,), dev)?;
        let seqlens_k = Tensor::from_vec(seqlens_k, (bs + 1,), dev)?;
        ...
        let sink = self.sinks.to_dtype(DType::F32)?; // [H]
        let out = flash_attn_blockmask_varlen_paged(
            &q_stacked,
            &k_paged,
            &k_paged,
            &seqlens_q,
        ...
        let attn_out = attn_out.reshape((bs, 1, num_heads, head_dim))?; // [B, 1, H, D]
        let grouped = attn_out.reshape((
            bs,
            1,
            self.cfg.o_groups,
        ...
        let grouped = self.o_a_proj.forward(&grouped)?;
        let grouped = grouped.reshape((bs, 1, self.cfg.o_groups * self.cfg.o_lora_rank))?;
        self.o_b_proj.forward(&grouped) // [B, 1, D]
    }

    /// Trim a paged (per-sequence, unbatch) KV tensor `[t, 1, D]` to the last
    /// `sliding_window - 1` rows.
    fn trim_paged(&self, kv: &Tensor) -> Result<Tensor> {
        let t = kv.dim(0)?;
        let keep = self.sliding_window.saturating_sub(1).min(t);
        if keep == t {
            Ok(kv.clone())
        } else {
            kv.narrow(0, t - keep, keep)
        }
    }
}
...
/// A snapshot of a compressor's per-sequence mutable state, used by the paged
/// continuous-batching decode path so a batch of sequences at different
/// absolute positions can share one compressor (weights) per layer while each
/// sequence carries its own compression history. `Csa` holds both the outer
/// CSA compressor state and its Lightning Indexer's state; `Hca` holds the
/// HCA state; `None` is the stateless sliding layer.
#[derive(Debug, Clone)]
enum DeepseekV4CompressorState {
    None,
    Csa {
        comp: CsaCompressionState,
        indexer: CsaCompressionState,
    },
    Hca(HcaCompressionState),
}

impl DeepseekV4Compressor {
    /// Snapshot the current per-sequence state of this compressor.
    fn state(&self) -> DeepseekV4CompressorState {
        match self {
            Self::Csa(c) => DeepseekV4CompressorState::Csa {
                comp: c.state.clone(),
                indexer: c.indexer.state.clone(),
            },
            Self::Hca(h) => DeepseekV4CompressorState::Hca(h.state.clone()),
        }
    }

    /// Overwrite this compressor's per-sequence state from a snapshot (used to
    /// load a specific sequence's compression history before `forward`).
    fn set_state(&mut self, s: &DeepseekV4CompressorState) {
        match (self, s) {
            (Self::Csa(c), DeepseekV4CompressorState::Csa { comp, indexer }) => {
                c.state = comp.clone();
                c.indexer.state = indexer.clone();
            }
            (Self::Hca(h), DeepseekV4CompressorState::Hca(st)) => h.state = st.clone(),
            (Self::Csa(_), DeepseekV4CompressorState::None)
            | (Self::Hca(_), DeepseekV4CompressorState::None) => {}
            _ => unreachable!("compressor state kind mismatch"),
        }
    }

    /// Run the compressor for a single sequence, loading its saved state first
    /// and persisting the updated state back. `hidden_states`/`q_residual` are
    /// the single-token `[1, 1, …]` slice for that sequence at its absolute
    /// position `seqlen_offset`; returns `(compressed_kv [1, 1, T, D],
    /// block_bias [1, 1, 1, T])` as `forward` does.
    fn paged_forward(
        &mut self,
        hidden_states: &Tensor,
        q_residual: &Tensor,
        seqlen_offset: usize,
        state: &mut DeepseekV4CompressorState,
    ) -> Result<(Tensor, Tensor)> {
        self.set_state(state);
        let out = self.forward(hidden_states, q_residual, seqlen_offset)?;
        *state = self.state();
        Ok(out)
    }
}
...
impl DeepseekV4DecoderLayer {
    ...
    /// Batched continuous-batching decode of one new token per sequence
    /// (`hidden_states` `[B, 1, D]`, `positions[B]` absolute), using the paged
    /// varlen/paged DSA flash kernel through `self_attn`. The MoE and mHC
    /// sublayers are position-agnostic and run batched as usual.
    fn forward_paged_decode(
        &mut self,
        hidden_states: &Tensor,
        positions: &[usize],
        layer_cache: &mut DeepseekV4PagedLayer,
        page_block_size: usize,
    ) -> Result<Tensor> {
        let dtype = hidden_states.dtype();
        let (post, comb, collapsed) = self.attn_hc.forward(hidden_states)?;
        ...
        let (post, comb, collapsed) = self.ffn_hc.forward(&hidden_states)?;
        let normed = self.post_attention_layernorm.forward(&collapsed)?;
        let mlp_output = self.mlp.forward(&normed, None)?;
        post.to_dtype(dtype)?
            .unsqueeze(D::Minus1)?
            .broadcast_mul(&mlp_output.unsqueeze(D::Minus2)?)?
            .broadcast_add(
...
mod tests {
    ...
    /// Paged continuous-batching decode parity vs eager, at the attention-layer
    /// level: a batch of sequences with DIFFERENT histories (varlen) and
    /// DIFFERENT absolute positions is decoded together through
    /// `DeepseekV4Attention::forward_paged_decode` (the varlen/paged DSA flash
    /// kernel, the `decode_batch` model API's attention core), and the output
    /// must match per-sequence eager single-batch decode at every batched step,
    /// for all three layer types (sliding, CSA, HCA). This is the correctness
    /// proof that the varlen/paged entry point is wired through the DeepseekV4
    /// model API.
    #[cfg(feature = "flash-attn")]
    fn check_varlen_paged_decode_parity(
        cfg: &DeepseekV4Config,
        label: &str,
        dev: &Device,
        prefills: &[usize],
        n_batched_steps: usize,
    ) -> candle::Result<()> {
        let n_seq = prefills.len();
        ...
        for layer in [
            ...
            // Flash attention driven through the paged path.
            let vb_flash = VarBuilder::from_tensors(merged.clone(), DType::BF16, dev);
            let mut paged = DeepseekV4Attention::new(&cfg, 0, true, vb_flash)?;
            // One eager attention per sequence (each with its own KV/compression).
            let mut eager: Vec<DeepseekV4Attention> = (0..n_seq)
                .map(|_| {
                    let vb = VarBuilder::from_tensors(merged.clone(), DType::BF16, dev);
                    DeepseekV4Attention::new(&cfg, 0, false, vb)
                })
                .collect::<Result<_>>()?;
            let mut cache = DeepseekV4PagedLayer::new(&cfg, 0, n_seq);
            // The paged path maps batch index -> slot, so a single-sequence
            // prefill occupies slot 0. Prefill sequences 1..n-1 first using slot 0
            // as a scratch slot (moving each result to its final slot and resetting
            // slot 0), then prefill sequence 0 last so it lands in slot 0 — this
            ...
            for step in 0..n_batched_steps {
                let positions: Vec<usize> = (0..n_seq).map(|i| prefills[i] + step).collect();
                let mut xs: Vec<Tensor> = Vec::with_capacity(n_seq);
                for i in 0..n_seq {
                    xs.push(
                        det_tensor(&[1, 1, cfg.hidden_size], (2000 + i * 200 + step) as f32)
                            .to_device(dev)?
                            .to_dtype(DType::BF16)?,
                    );
                }
                ...
                for i in 0..n_seq {
                    let e = eager[i]
                        ...
                        .to_dtype(DType::F32)?;
                    let p = paged_out.narrow(0, i, 1)?.to_dtype(DType::F32)?;
                    assert_close(
                        &e,
                        &p,
                        1e-1,
    ...
    /// Paged continuous-batching decode parity at the tiny-model shape (head_dim
    /// 128, 8 heads, MQA Hkv=1): a varlen batch of 3 sequences (prefills 4 /
    /// 150 / 400 tokens; the longest grows past a 64-token physical page for
    /// CSA) decoded together over 20 batched steps.
    #[cfg(feature = "flash-attn")]
    #[test]
    fn varlen_paged_batched_decode_parity_head_dim_128() -> candle::Result<()> {
        let dev = match Device::new_cuda(0) {
            Ok(d) => d,
            Err(e) => {
                eprintln!("no CUDA device; skipping varlen/paged decode parity head_dim 128 ({e})");
                return Ok(());
            }
        };
        let mut cfg = decode_parity_config();
        ...
        cfg.q_lora_rank = 128;
        cfg.o_lora_rank = 128;
        check_varlen_paged_decode_parity(&cfg, "tiny-head_dim-128", &dev, &[4, 150, 400], 20)
    }

    /// Paged continuous-batching decode parity at the real V4-Flash shape
    /// (head_dim 512, 64 heads, MQA Hkv=1): a varlen batch of 3 sequences
    /// (prefills 4 / 100 / 300) decoded together over 10 batched steps.
    #[cfg(feature = "flash-attn")]
    #[test]
    fn varlen_paged_batched_decode_parity_v4flash_512_64h() -> candle::Result<()> {
        let dev = match Device::new_cuda(0) {
            Ok(d) => d,
            Err(e) => {
                eprintln!("no CUDA device; skipping varlen/paged decode parity 512/64 ({e})");
                return Ok(());
            }
        };
        let mut cfg = decode_parity_config();
        ...
        cfg.q_lora_rank = 512;
        cfg.o_lora_rank = 512;
        check_varlen_paged_decode_parity(&cfg, "v4flash-512-64h", &dev, &[4, 100, 300], 10)
    }
